...
module kotnShadowImageNames{
	...
	export function itemBeforeSubmit(type) {
	    if (type == 'edit' || type == 'create' || type == 'xedit') {
	        ...
	        if (imageName && imageName != shadowImage) nlapiSetFieldValue('custitem_kotn_image_name', imageName);
	        ...
	        if (thumbnailName && thumbnailName != shadowthumbnail) nlapiSetFieldValue('custitem_kotn_thumbnail_name', thumbnailName);
	    }
	}
