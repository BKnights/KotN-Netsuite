...
/*
Utility Functions
copyright 2008 - 2010 Brett Knights. All rights reserved. This material may not be reproduced,
...
interface JobContext {getTargetsUsage: () =>number; getMaxUsage : () => number;}

function batchProcess(getContext :(origContext : JobContext) => JobContext, getTargets: (ctx:JobContext, isRerunCheck: bool) => Object[], processTarget : (ctx : JobContext, simpleArg? : any) => bool, rerunParams : (ctx :JobContext) => void){
	...
	try{
		...
		each(targets, function(idx, target){
			...
			if(nlapiGetContext().getExecutionContext() == "scheduled"){
					...
					}else nlapiGetContext().setPercentComplete( (100*( 1 -(remainUsage/targetStartUsage))).toFixed(1));
			}
			...
			if(!canGo) nlapiLogExecution('AUDIT', 'Processing Targets', 'processed: ' + targetCount +' targets');
		...
		if(remainder && remainder.length){
			if(nlapiGetContext().getExecutionContext() == "scheduled") { //check executionContext to avoid invalid calls in debugger
			...
			}else nlapiLogExecution('AUDIT', 'Processing Targets', "finished non-scheduled run " + remainder.length + " targets left to process.");
		}
	}catch(e){
